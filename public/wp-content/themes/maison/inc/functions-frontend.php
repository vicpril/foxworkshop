...
if ( ! function_exists( 'maison_category' ) ) {
	function maison_category( $post ) {
		// format
...
if ( ! function_exists( 'maison_center_meta' ) ) {
	function maison_center_meta( $post ) {
		// format
		$post_format = get_post_format();
		...
		if(!is_single()){
			...
			echo "<span class='author'>".esc_html__( ' / By: ', 'maison' ).'<a href='.esc_url(get_author_posts_url( $id )).'>'.get_the_author().'</a>' .'</span>';
			echo '</div>';
		echo '</div>';
	}
}
...
if ( ! function_exists( 'maison_full_top_meta' ) ) {
	function maison_full_top_meta( $post ) {
		// format
		$post_format = get_post_format();
		$header_class = $post_format ? '' : 'border-left';
		...
		if ( ! is_search() ) {
			if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				...
				comments_popup_link( esc_html__( '0 comments', 'maison' ), esc_html__( '1 comment' , 'maison' ), esc_html__( '% comments', 'maison' ) );
				echo '</span>';
			}
		}
		echo '</header>';
...
if ( ! function_exists( 'maison_post_tags' ) ) {
	function maison_post_tags() {
		$posttags = get_the_tags();
		if ( $posttags ) {
			echo '<span class="entry-tags-list"><strong>'.esc_html__( 'Tags: ' , 'maison' ).'</strong> ';
			$i = 1;
			$size = count( $posttags );
...
if ( ! function_exists( 'maison_post_format_link_helper' ) ) {
	function maison_post_format_link_helper( $content = null, $title = null, $post = null ) {
		if ( ! $content ) {
			$post = get_post( $post );
			$title = $post->post_title;
			$content = $post->post_content;
		}
		$link = maison_get_first_url_from_string( $content );
		if ( ! empty( $link ) ) {
...
if ( !function_exists('maison_get_page_title') ) {
	function maison_get_page_title() {
		$title = '';
		if ( !is_front_page() || is_paged() ) {
			...
			if ( is_home() ) {
				$title = esc_html__( 'The Blogs', 'maison' );
			} elseif (is_category()) {
				global $wp_query;
				...
				if ( get_post_type() != 'post' ) {
					...
					$title = esc_html__( 'The Blog', 'maison' );
				}
				...
				$title = esc_html__('Search results for "','maison')  . get_search_query();
			} elseif ( is_tag() ) {
				$title = esc_html__('Posts tagged "', 'maison'). single_tag_title('', false) . '"';
			} elseif ( is_author() ) {
				global $author;
				$userdata = get_userdata($author);
				$title = $userdata->display_name;
			} elseif ( is_404() ) {
				$title = esc_html__('Error 404', 'maison');
			}
		}
		return $title;
	}
...
if ( ! function_exists( 'maison_breadcrumbs' ) ) {
	function maison_breadcrumbs() {

		$delimiter = ' ';
		$home = esc_html__('Home', 'maison');
		$before = '<li><span class="active">';
		$after = '</span></li>';
		$title = maison_get_page_title();
		if ( !is_front_page() || is_paged()) {
			global $post;
			$homeLink = esc_url( home_url() );
			...
			if (is_category()) {
				...
				if ( get_post_type() == 'give_forms' ) {
					echo trim($before) . esc_html__( 'Our Campaigns', 'maison' ) . $after;
				} elseif ( get_post_type() == 'simple_event' ) {
					echo trim($before) . esc_html__( 'Our Events', 'maison' ) . $after;
				...
				echo trim($before) . esc_html__('Search results for "','maison')  . get_search_query() . '"' . $after;
			} elseif ( is_tag() ) {
				echo trim($before) . esc_html__('Posts tagged "', 'maison'). single_tag_title('', false) . '"' . $after;
			} elseif ( is_author() ) {
				global $author;
				$userdata = get_userdata($author);
				echo trim($before) . esc_html__('Articles posted by ', 'maison') . $userdata->display_name . $after;
			} elseif ( is_404() ) {
				echo trim($before) . esc_html__('Error 404', 'maison') . $after;
			} elseif ( is_home() ) {
				echo trim($before) . esc_html__('The Blogs', 'maison') . $after;
			}
...
if ( ! function_exists( 'maison_render_breadcrumbs' ) ) {
	function maison_render_breadcrumbs() {
		global $post;
		...
		if ( is_page() && is_object($post) ) {
			...
			$show = maison_get_config('show_blog_breadcrumbs', true);
			if ( !$show  ) {
				return ''; 
			}
			$breadcrumb_img = maison_get_config('blog_breadcrumb_image');
	        $breadcrumb_color = maison_get_config('blog_breadcrumb_color');
	        $style = array();
	        if ( $breadcrumb_color ) {
...
if ( !function_exists( 'maison_print_style_footer' ) ) {
	function maison_print_style_footer() {
    	$footer = maison_get_footer_layout();
    	if ( $footer ) {
    		$args = array(
				'name'        => $footer,
...
if ( ! function_exists( 'maison_paging_nav' ) ) {
	function maison_paging_nav() {
		...
		$links = paginate_links( array(
			...
			'prev_text' => esc_html__( '', 'maison' ),
			'next_text' => esc_html__( '', 'maison' ),
		) );
		...
		<nav class="navigation paging-navigation" role="navigation">
			<h1 class="screen-reader-text hidden"><?php esc_html_e( 'Posts navigation', 'maison' ); ?></h1>
...
if ( ! function_exists( 'maison_post_nav' ) ) {
	function maison_post_nav() {
		...
		<nav class="navigation post-navigation" role="navigation">
			<h3 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'maison' ); ?></h3>
			<div class="nav-links clearfix">
				<?php
				if ( is_attachment() ) :
					previous_post_link( '%link','<div class="col-lg-6"><span class="meta-nav">'. esc_html__('Published In', 'maison').'</span></div>');
				else :
					previous_post_link( '%link','<div class="pull-left"><span class="meta-nav">'. esc_html__('Previous Post', 'maison').'</span></div>' );
					next_post_link( '%link', '<div class="pull-right"><span class="meta-nav">' . esc_html__('Next Post', 'maison').'</span><span></span></div>');
				endif;
				?>
			</div><!-- .nav-links -->
		</nav><!-- .navigation -->
...
if ( !function_exists('maison_pagination') ) {
    function maison_pagination($per_page, $total, $max_num_pages = '') {
    	global $wp_query, $wp_rewrite;
        ?>
        <div class="apus-pagination">
        	<?php
        	$prev = esc_html__('<','maison');
        	$next = esc_html__('>','maison');
        	$pages = $max_num_pages;
        	$args = array('class'=>'');
...
if ( !function_exists('maison_comment_form') ) {
	function maison_comment_form($arg, $class = 'btn-white ') {
		global $post;
...
if (!function_exists('maison_list_comment') ) {
	function maison_list_comment($comment, $args, $depth) {
...
function maison_display_footer_builder($footer) {
...
function maison_comment_field_to_bottom( $fields ) {
	$comment_field = $fields['comment'];
	unset( $fields['comment'] );
	$fields['comment'] = $comment_field;
	return $fields;
}
add_filter( 'comment_form_fields', 'maison_comment_field_to_bottom' );

function maison_get_blogs_layout_type() {
	$layout = maison_get_config( 'blog_display_mode', 'grid' );
	$layout = !empty($layout) ? $layout : 'grid';
	return $layout;
}

function maison_get_blog_thumbsize() {
	$thumbsize = maison_get_config( 'blog_item_thumbsize', 'full' );
	return $thumbsize;
}
 ...
 */
function maison_create_placeholder($size) {
...
function maison_display_image($img) {
	if ( !empty($img) && isset($img[0]) ) {
		if (maison_get_config('image_lazy_loading')) {
			$placeholder_image = maison_create_placeholder(array($img[1], $img[2]));
			?>
